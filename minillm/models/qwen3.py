"""Qwen3 模型实现（单卡版本，无 TP）。

支持 Qwen3 系列模型。
与 Qwen2 的主要差异：
- 可选的 Q/K RMSNorm（当 qkv_bias=False 时启用）
- 默认 rope_theta=1000000
- qkv_bias 默认为 False
"""
...
class Qwen3Attention(nn.Module):
    ...
    Qwen3 使用 GQA（Grouped Query Attention），可选 Q/K RMSNorm。
    当 qkv_bias=False 时，对 Q 和 K 分别应用 RMSNorm。
    ...
    Attributes:
        ...
        q_norm: Q 归一化（仅 qkv_bias=False 时存在）
        k_norm: K 归一化（仅 qkv_bias=False 时存在）
    """
    ...
    def __init__(
        ...
        # get_rope 的 @lru_cache 要求参数可哈希，但 Qwen3 的 rope_scaling 可能是 dict。
        # 当 rope_type 为 "default" 时等同于无缩放，直接置 None。
        ...
        # Qwen3 特有：当 qkv_bias=False 时启用 Q/K RMSNorm
    ...
    def forward(
        ...
        # Qwen3 特有：在 RoPE 前应用 Q/K RMSNorm
        if not self.qkv_bias:
            q = self.q_norm(q)
            k = self.k_norm(k)
...
class Qwen3MLP(nn.Module):
    """Qwen3 MLP 层（SwiGLU 激活）。

    与 Qwen2 的 MLP 结构完全相同。
...
class Qwen3DecoderLayer(nn.Module):
    ...
    与 Qwen2 的 DecoderLayer 骨架相同，但传入 Qwen3 特有参数。
